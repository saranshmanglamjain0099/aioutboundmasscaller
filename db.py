...
# In production (Docker/Coolify): set via VPS env vars — single source of truth.
# In local dev: load_dotenv(override=False) fills in from .env file.
# ---------------------------------------------------------------------------

# Hardcoded defaults only for non-secret config with sensible fallbacks
_FALLBACKS = {
...
def _default(key: str) -> str:
    """Always read from os.getenv at call time — never from a frozen dict."""
    return os.getenv(key, _FALLBACKS.get(key, ""))
